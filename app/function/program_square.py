...
def program_square(self_window=None):
  if self_window is not None:
    # закрываем передано окно
    self_window.destroy()
    print("parent window is closed")

  def donothing():
    window = Toplevel(program1)
    window.title("Square filter")
  ...
  def get_program_ready(show_error=False, output_in_console=True):
    ...
    laser_radius = field_laser_radius.get()
    error = False
    ...
    if not is_float(laser_radius) or not is_int(laser_radius):
      if show_error:
    ...
    if output_in_console:
      ...
      print("laser_radius = " + laser_radius)
  ...
  def load_images():
    ...
    if get_program_ready(show_error=True, output_in_console=False):
      # photo_num = int(field_amount_scanned_photos.get())
      # scan_photos = []

      # if is_float(photo_num) or is_int(photo_num):
      scan_photos = filedialog.askopenfilenames(initialdir="/", title="Select file",
                                                filetypes=(("bmp files", "*.bmp"), ("all files", "*.*")))
      # старый вариант
      # if is_float(photo_num) or is_int(photo_num):
      #   for i in range(photo_num):
      #     program1.filename = filedialog.askopenfilename(initialdir="/", title="Select file",
      #                                                    filetypes=(("bmp files", "*.bmp"), ("all files", "*.*")))
      #     scan_photos.insert(i, program1.filename)
    ...
    if get_program_ready(show_error=True, output_in_console=False):
      ...
      if amount_of_photos_scan > 0:
        ...
        for i in range(amount_of_photos_scan):
          ...
          R[i] = (R[i] / float(field_amount_of_pixels.get())) + float(field_laser_radius.get())
        ...
        plt.plot(tg)
        plt.title(u"Тангенс краевого угла видеоскан")
        plt.show()

  # def Camera():
  #   global scan_photos
  #   print("Camera")
  #   amount_of_photos_camera = len(scan_photos)
  #
  #   if amount_of_photos_camera > 0:
  #     if get_program_ready(show_error=True, output_in_console=False):
  #       PictList = []
  #       b = []
  ...
  #
  #       for key, photo in enumerate(scan_photos):
  #         PictList.insert(key, Image.open(photo))
  #
  ...
  # filemenu.add_command(label="Close", command=program1.quit)
  filemenu.add_command(label="Выход", command=program1.quit)
  ...
  program1.config(menu=menubar)

  program1.title("Программа по обработке изображений")
  program1.geometry('1080x600')
  program1.config(bg='#FFE4B5')  # цвет поля
  program1.resizable(False, False)  # отмена ресайза окна

  print("Program \"Program1\" run!\n\n")
  ...
  label_parameter_z = Label(program1, text='Укажите значение параметра z', fg='yellow', bg='#9932CC')
  ...
  label_error_parameter_z = Label(program1, fg='#FFE4B5', bg='#FFE4B5')
  ...
  label_parameter_n = Label(program1, text='Введите значение показателя n', fg='yellow', bg='#9932CC')
  ...
  label_error_indicator_n = Label(program1, fg='#FFE4B5', bg='#FFE4B5')
  ...
  label_threshold_filter = Label(program1, text='Введите значение порогового фильтра', fg='yellow', bg='#9932CC')
  ...
  label_error_threshold_filter = Label(program1, fg='#FFE4B5', bg='#FFE4B5')
  ...
  # label_number_scanned_photos = Label(program1, text='Введите кол-во фотографий с видеоскана', fg='yellow',
  #                                     bg='#9932CC')
  ...
  label_error_amount_scanned_photos = Label(program1, fg='#FFE4B5', bg='#FFE4B5')
  ...
  # label_camera_photo = Label(program1, text='Введите кол-во фотографий с камеры', fg='yellow', bg='#9932CC')
  ...
  label_error_amount_camera_photo = Label(program1, fg='#FFE4B5', bg='#FFE4B5')
  ...
  label_amount_pixels_per_millimeter = Label(program1, text='Введите кол-во пикселей в одном мм', fg='yellow',
                                             bg='#9932CC')
  ...
  label_error_amount_pixels_per_millimeter = Label(program1, fg='#FFE4B5', bg='#FFE4B5')
  ...
  label_laser_radius = Label(program1, text='Введите радиус лазерного пучка', fg='yellow', bg='#9932CC')
  ...
  label_error_laser_radius = Label(program1, fg='#FFE4B5', bg='#FFE4B5')
  ...
  start_data_input = Button(program1, text='Ввод и проверка значений', command=validation_required_values)
  start_data_input.config(width=23, height=2, bg='#9ACD32', fg='black')
  start_data_input.place(x=600, y=400)

  # # Загрузка фото
  button_photos_upload = Button(program1, text='Загрузить фото', command=load_images)
  button_photos_upload.config(width=23, height=2, bg='#9ACD32', fg='black')
  button_photos_upload.place(x=200, y=250)

  # Canny_filter
  # start_Canny_filter = Button(program1, text='Фильтр Кэнни', command=Canny_filter)
  # start_Canny_filter.config(width=23, height=2, bg='#9ACD32', fg='black')
  # start_Canny_filter.place(x=400, y=300)
